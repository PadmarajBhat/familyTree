    ...
    const saveWithMerge = useCallback(async (
        ...
        if (isSheetsMode) {
            ...
            try {
                ...
                // 5. Append ChangeLog (History)
                const changeLog = {
                    editedBy: localTree.meta.createdBy || 'Unknown', // Or usage of specific user context if available
                    editedTime: new Date().toISOString(),
                    changes: summaryText,
                    rootNodeName: localTree.nodes[localTree.rootNodeId]?.name || 'Unknown',
                    // Optional: structured data could be passed if generated by the caller. For now, simple text.
                    structured: []
                };
                // Dynamically import to avoid circular dependency if needed, or better, add to imports.
                // Assuming imported 'appendChangeLogToSheets' from services/drive
                syncTasks.push(appendChangeLogToSheets(changeLog));
