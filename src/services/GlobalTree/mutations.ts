...
import { acquireLock, releaseLock, getFileContent, updateTreeFile, listTreeFiles } from '../drive';
...
export async function addSpouseToRemoteNode(treeId: string, nodeId: string, newSpouseId: string, userEmail: string): Promise<boolean> {
...
export async function updateRemoteNode(treeId: string, nodeId: string, updates: Partial<PersonNode>, userEmail: string): Promise<boolean> {
...
export async function removeLinksToTree(deletedTreeId: string, userEmail: string, onProgress?: (msg: string) => void): Promise<void> {
    console.log(`Starting deep cleanup for tree ${deletedTreeId}...`);
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const files = (await listTreeFiles()) as any[];
        const otherFiles = files.filter((f: any) => f.id !== deletedTreeId && !f.name.startsWith('backup_') && !f.name.startsWith('delete_'));
