...
export const searchMembers = (nodes: PersonNode[], query: string): PersonNode[] => {
    ...
    return nodes.filter(node => {
        ...
        // 1.5 Name Translations match
        if (node.nameTranslations) {
            const translations = Object.values(node.nameTranslations).filter(Boolean) as string[];
            if (translations.some(t => t.toLowerCase().includes(lowerQuery))) return true;
        }
